...
public class Billard_Tracker implements PlugInFilter
{
    ...
    {   int w1 = ip1.getWidth();
        ...
        {   
            ...
            {
            	int G1 = pix1[i1] & 0xff;
            	int G2 = pix1[i1 + w1 + 1] & 0xff;
            	int green = (G1 + G2) >> 1;
            	int blue = pix1[i1 + 1] & 0xff;
            	int red = pix1[i1 + w1] & 0xff;
            	
            	hsbValues = Color.RGBtoHSB(red, green, blue, null);
            	
            	pixGray[y * w2 + x] = (byte) (hsbValues[2] * 255f);
            	pixRGB[y * w2 + x] = ((red & 0xff)<<16)+((green & 0xff)<<8) + (blue & 0xff);
            	pixHue[y * w2 + x] = (byte) (hsbValues[0] * 255f);
    ...
    public static void main(String[] args)
    {
        ...
        ImagePlus im = new ImagePlus("../../CPVR/CPVR/Images/Billard2048x1088x1.png");
        im.show();
        plugin.setup("", im);
